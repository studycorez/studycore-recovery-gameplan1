...
export function computeProgram(diagnosticEntries, studentInfo) {
  ...
  const { plan: fullWeeklyPlan, totalSessions: fullSessionsNeeded } = buildWeeklyPlan(topicSequence, sessionsPerWeek, weeklySchedule);
...
function buildWeeklyPlan(topicSequence, sessionsPerWeek, weeklySchedule) {
  const sessions = [];
  let contentCount  = 0;
