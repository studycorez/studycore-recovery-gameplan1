...
export function computeProgram(diagnosticEntries, studentInfo) {
  ...
  // ── Compute available sessions from test date ─────────────────────────────
  ...
  // Derive max topics from available sessions (leave room for Phase 3-5 overhead + pacing/WAE)
  const pacingSlots = blanks > 5 ? 1 : 0;
  const waeSlots    = 1; // WAE inserted after 5 R&W topics, assume 1
  const maxTopicsFromDate = availableSessions != null
    ? Math.max(1, Math.floor(
        (availableSessions / (1 + SHARED_PHASE_OVERHEAD) - pacingSlots - waeSlots)
        / SESSIONS_PER_TOPIC
      ))
    : 20;
  ...
  // Also respect the date-constrained cap so the plan fits before the test.
  const topicCap = Math.min(20, maxTopicsFromDate);
  ...
  for (const topic of topicsWithMisses) {
    ...
    if (selectedTopics.length >= topicCap) break;
  }
  ...
  const programSummary = {
    ...
    weeksUntilTest,
    availableSessions,
    isDateConstrained: availableSessions != null && totalSessionsNeeded > availableSessions,
    feasibility,
    targetGain,
