...
function buildWeeklyPlan(topicSequence, sessionsPerWeek, weeklySchedule, projectionParams) {
  ...
  let programPhaseNum        = 1; // increments after each Phase 4 (practice test) session
  ...
  for (const t of topicSequence) {
    ...
    if (phase4Buffer >= 6) {
      ...
      sessions.push({
        ...
        sessionBlurb: 'Full score-report review — tutor maps every error back to a taught topic, logs your conversion rate, and flags any topics needing a Phase 5 re-teach.',
        isCheckpoint: true,
        programPhase: programPhaseNum,
      });
      phase4Buffer = 0;
      programPhaseNum++; // next sessions belong to the following program phase
    }
  }
  ...
  for (let i = 0; i < phase5Count; i++) {
    sessions.push({
      ...
      sessionBlurb: 'Targeted re-teach on the Phase 4 flagged sub-skill. Tutor brings 15–20 fresh questions drilled specifically on that one gap until 85%+ is confirmed.',
      isCheckpoint: false,
      programPhase: programPhaseNum,
    });
  }
  ...
  for (let i = plan.length - 1; i >= 0; i--) {
    if (plan[i].phase === 'Phase 4') {
      ...
      plan[i].sessionBlurb = 'Benchmark review session — score compared against your baseline, final 2-week push is scoped, and remaining weak points are ranked by impact.';
      break;
    }
  }
