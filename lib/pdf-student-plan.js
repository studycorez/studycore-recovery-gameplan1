  ...
  if (weeklyPlan && weeklyPlan.length > 0) {
    ...
    for (const sess of weeklyPlan) {
      ...
      if (pp !== curProgramPhase) {
        ...
        renderItems.push({ type: 'phaseStart', phaseNum: pp, topicCount });
      }
      // Week header
        ...
        <Page size="LETTER" style={s.page}>
          ...
          {renderItems.map((item, idx) => {
            if (item.type === 'phaseStart') {
              const phaseNames = { 1: 'Build', 2: 'Advance', 3: 'Consolidate', 4: 'Finalise' };
              ...
              return (
                <View key={idx} style={s.phaseHeader}>
                  <Text style={s.phaseHeaderLabel}>PROGRAM PHASE {item.phaseNum} — {phaseName.toUpperCase()}{topicLabel}</Text>
                  <Text style={s.phaseHeaderSub}>
                    {item.phaseNum === 1
                      ? 'Learn each topic · pass untimed gate'
                      : item.phaseNum === 2
                      ? 'Speed drills · close remaining gaps'
                      : item.phaseNum === 3
                      ? 'Mixed practice · lock in retention'
                      : 'Final push · verify readiness'}
                  </Text>
                </View>
              );
            }
