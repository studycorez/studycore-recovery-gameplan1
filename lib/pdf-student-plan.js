...
function StudentDocument({ data }) {
  ...
  const freqLabel = sessionFrequency === '2x' ? '2 sessions per week' : '1 session per week';
      ...
      <Page size="LETTER" style={s.page}>
        ...
        <View style={[s.callout, { backgroundColor: C.lightBlue, borderLeftColor: C.blue }]}>
          ...
          <Text style={s.calloutBody}>
            {programSummary.topicsToTeach} topics · {programSummary.totalSessionsNeeded} sessions · {
              programSummary.weeksUntilTest != null
                ? `${programSummary.weeksUntilTest} weeks until test`
                : `${programSummary.weeksNeeded} weeks`
            } · {freqLabel}
            {programSummary.weeksUntilTest != null && programSummary.availableSessions != null
              ? `\n\nYou have ${programSummary.availableSessions} sessions available before your test date at ${freqLabel}.`
              : ''}
