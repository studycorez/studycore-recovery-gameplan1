...
const s = StyleSheet.create({
  ...
  scheduleColSession: { width: 52, fontSize: 8, color: C.textLight },
  scheduleColPhase:   { width: 100, fontSize: 8, fontFamily: 'Helvetica-Bold' },
  scheduleColTopic:   { flex: 1, fontSize: 8, color: C.textMid },
...
function phaseInfo(sess) {
  ...
  if (sess.phase === 'Phase 3') return { label: 'Error Pattern Analysis',      color: C.green };
      ...
      {weekGroups.length > 0 && (
        <Page size="LETTER" style={s.page}>
          ...
          <View style={{ flexDirection: 'row', backgroundColor: C.blue, paddingHorizontal: 8, paddingVertical: 5, borderRadius: 3, marginBottom: 4 }}>
            <Text style={{ color: C.white, fontSize: 7.5, fontFamily: 'Helvetica-Bold', width: 52 }}>Session</Text>
            <Text style={{ color: C.white, fontSize: 7.5, fontFamily: 'Helvetica-Bold', width: 100 }}>Focus</Text>
          ...
          {weekGroups.map((wg) => (
            <View key={wg.week} wrap={false}>
              ...
              {wg.sessions.map((sess, si) => {
                ...
                return (
                  <View key={si} style={[s.scheduleRow, si % 2 === 1 && s.scheduleRowAlt]}>
