...
function StudentContractDocument({ data }) {
  const {
    ...
    totalInvestment, paymentStructure, paymentPlan,
    refundPolicy, hasGuarantee,
    signedName, signedAt,
  } = data;
      ...
      <Page size="LETTER" style={styles.page}>
        ...
        <Section number={1} title="PARTIES & PROGRAM DETAILS">
          ...
          <Text style={[styles.paragraph, { marginTop: 8 }]}>
            <Text style={styles.bold}>Written Notice: </Text>For purposes of this Agreement, "written notice" means a communication delivered via email to the addresses on file or via text message (SMS) to the phone numbers on file. Notice is deemed received upon successful delivery.
          </Text>
        </Section>
        ...
        <Section number={2} title="PROGRAM SCOPE & SCHEDULE">
          ...
          <Text style={styles.paragraph}>
            <Text style={styles.bold}>Additional Sessions: </Text>If Student wishes to purchase additional 1-on-1 sessions beyond those included in this Agreement (outside of the performance guarantee period), Client may do so at the standard per-session rate by written agreement with StudyCore. Any such addition will be governed by the terms of this Agreement.
          </Text>
        </Section>
        ...
        <Section number={4} title="PAYMENT TERMS">
          ...
          <Detail label="Platform Fee (Non-refundable)" value="$200.00 — 1 year platform access from program start" />
          <Detail label="Payment Structure" value={paymentStructure} />
          {paymentStructure === 'Payment Plan' && paymentPlan && (
            <Text style={[styles.paragraph, { marginTop: 6 }]}>
              <Text style={styles.bold}>Installment Schedule: </Text>{paymentPlan}
            </Text>
          )}
          ...
          <Text style={styles.paragraph}>
            <Text style={styles.bold}>Payment Failure: </Text>If any scheduled payment fails, StudyCore will notify Client via email or text within 2 business days. Client has five (5) business days from notification to resolve the failed payment. Sessions will continue during this grace period. If unresolved after 5 business days, sessions will be paused until the account is brought current. StudyCore is not liable for delays in Student's preparation resulting from a program pause due to non-payment.
          </Text>
          <Text style={styles.paragraph}>
        ...
        <Section number={5} title="CANCELLATION & REFUND POLICY">
          ...
          <Text style={styles.paragraph}>
            <Text style={styles.bold}>StudyCore-Initiated Cancellations: </Text>If StudyCore or the assigned tutor must cancel a scheduled session, StudyCore will notify Client as soon as possible and coordinate a makeup session at a mutually agreeable time. Makeup sessions will be scheduled promptly and at no additional cost to Client.
          </Text>
          {refundPolicy === 'prorated' ? (
            <Text style={styles.paragraph}>
              <Text style={styles.bold}>Discontinuation: </Text>If Client chooses to discontinue the program at any point, StudyCore will provide a prorated refund based on the number of 1-on-1 sessions completed. Refund = (Total Investment − $200 Platform Fee) × (sessions remaining / total sessions). The $200 Platform Fee is non-refundable under all circumstances, as platform access is granted immediately upon signing.
            </Text>
          ) : (
            <Text style={styles.paragraph}>
              <Text style={styles.bold}>Discontinuation: </Text>If Client discontinues within the first three (3) 1-on-1 sessions, StudyCore will issue a full refund of the program fee (Total Investment minus the non-refundable $200 Platform Fee). After the third 1-on-1 session has been delivered, no refund is available. The $200 Platform Fee is non-refundable under all circumstances, as platform access is granted immediately upon signing.
            </Text>
          )}
        </Section>
        ...
        {hasGuarantee ? (
          ...
          <Section number={6} title="ACKNOWLEDGMENT — NO PERFORMANCE GUARANTEE">
            <Text style={styles.paragraph}>
              This program does not include a performance guarantee. Client acknowledges that while StudyCore will deliver all services described herein and work diligently toward Student's target score of {targetScore || '—'}, StudyCore makes no guarantee that Student will achieve their target SAT score. Results depend on student effort, engagement, and factors outside StudyCore's control.
            </Text>
          </Section>
        )}
        ...
        <Section number={7} title="CLIENT RESPONSIBILITIES">
          <Text style={styles.paragraph}>
            {hasGuarantee
              ...
              : 'Client and Student agree to the following.'}
          </Text>
          ...
          <Bullet>Complete 100% of assigned homework, practice tests, and drill sets between sessions.{hasGuarantee ? ' StudyCore tracks homework completion via session reports submitted by the assigned tutor. The Performance Guarantee is void if compliance falls below 100%.' : ''}</Bullet>
          <Bullet>Complete all assigned practice tests independently outside of sessions, under timed conditions</Bullet>
          <Bullet><Text style={styles.bold}>SAT Registration: </Text>Client must register Student for the Target SAT Test ({targetTestDate}) within four (4) weeks of the program start date and provide StudyCore with confirmation of registration.{hasGuarantee ? ' Failure to register within this window voids the Performance Guarantee.' : ''}</Bullet>
        ...
        <Section number={10} title="SESSION RECORDING & COMMUNICATIONS CONSENT">
          ...
          <Text style={styles.paragraph}>
            <Text style={styles.bold}>Score Release Authorization: </Text>Client authorizes StudyCore LLC to review Student's official College Board score reports as needed to administer this Agreement, including verification of performance guarantee eligibility and program outcome tracking. Client agrees to provide StudyCore with the official score report following the Target SAT Test.
          </Text>
        </Section>
