  ...
  return (
    <div>
      ...
      <PS title="06 — Payment & Performance Bonus">
        <p>Rate per General Agreement. <strong>The $200.00 is a fixed cost to StudyCore for this assignment:</strong> if {ts.studentName || 'Student'} hits their target score of {ts.targetScore || '—'} on their first SAT after the program, it is paid to Tutor as a performance bonus and Tutor advances to the next rate tier ($20→$22→$25→$27→$30→$32→$35 max), taking effect the next pay period after score confirmed. If the performance guarantee is triggered (see Section 07), the $200 is instead allocated to fund the guarantee session period and Tutor forfeits the bonus for this assignment. Refund clause: no pay for first 3 sessions if student discontinues; +1 strike if Tutor's fault.</p>
      </PS>
      <PS title="07 — Performance Guarantee Obligation">
        <p>If {ts.studentName || 'Student'} triggers the guarantee and Tutor is still active, Tutor continues sessions at $20/hr funded by the $200 guarantee allocation from Section 06. No new contract required. <strong>Guarantee Session Cap:</strong> Tutor's obligation is capped at 10 additional sessions — after 10 sessions beyond the original End Date, Tutor's obligation ends regardless of whether the target score was achieved. <strong>Performance Review:</strong> Guarantee trigger initiates a session report review. Tutor-side deficiencies = 1 strike. Clean review = no consequence.</p>
      </PS>
