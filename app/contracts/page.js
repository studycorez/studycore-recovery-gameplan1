  ...
  return (
    <div>
      ...
      <PS title="06 — Payment & Performance Bonus">
        <p>Rate per General Agreement. <strong>Performance Bonus & Rate Progression:</strong> If {ts.studentName || 'Student'} hits their target score of {ts.targetScore || '—'} on their first SAT after the program, Tutor receives a $200 cash bonus and advances to the next rate tier ($20→$22→$25→$27→$30→$32→$35 max), taking effect the next pay period after score confirmed. If the performance guarantee is triggered (see Section 07), Tutor forfeits the $200 bonus for this assignment. Refund clause: no pay for first 3 sessions if student discontinues; +1 strike if Tutor's fault.</p>
      </PS>
      <PS title="07 — Performance Guarantee Obligation">
        <p>If {ts.studentName || 'Student'} triggers the guarantee and Tutor is still active, Tutor continues sessions at $20/hr beyond the original End Date. No new contract required. <strong>Duration:</strong> Length of obligation is at StudyCore's sole discretion — StudyCore will notify Tutor in writing when the obligation is fulfilled or concluded. <strong>Performance Review:</strong> Guarantee trigger initiates a session report review. Tutor-side deficiencies = 1 strike. Clean review = no consequence.</p>
      </PS>
